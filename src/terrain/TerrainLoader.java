...
public class TerrainLoader
{
    ...
    public static RawData generateTerrain(String height_map, String texture)
    {
        BufferedImage image = null;
        try
        {
            image = ImageIO.read(new File("res/textures/" + height_map));
        } catch (IOException e)
        {
            e.printStackTrace();
        }
        ...
        for (int i = 0; i < vertexCount; i++)
        {
            for (int j = 0; j < vertexCount; j++)
            {
                ...
                textureArray[v * 2 + 1] = i / (vertexCount - 1);
